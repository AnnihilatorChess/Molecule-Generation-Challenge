"""v1: Sample, post-process, verify against the OFFICIAL aux metrics, then evaluate
local FCD.

The grading rule: validity, uniqueness, novelty all >= 0.9, or no positive grade.
Our pipeline aims for 1.0 on all three by construction — but the only way to be
sure is to reproduce the official evaluator's checks on the exact file we will
submit. This script:

  1. Adaptively samples enough sequences that, after RDKit-canonicalize → dedup →
     drop train overlaps, at least `target_n` SMILES survive.
  2. Writes the submission file.
  3. Re-runs the official validity/uniqueness/novelty computation on that file
     (mirrors `evaluation.evaluate_submission.get_metric` exactly).
  4. Asserts each aux metric >= 0.9 (or the user-specified threshold).
  5. Computes local FCD against the held-out reference.

Any mismatch between our post-process and the official eval surfaces in step 3
instead of after a submission is burned.
"""
...
def adaptive_sample(
    ...
    """Sample SMILES in chunks, accumulating a canonical-deduped novel pool until
    we have at least `target_n` survivors. Returns (submission, stats)."""
    pool_set: set[str] = set()
    pool: list[str] = []
    total_raw = 0
    total_decoded = 0
    total_valid = 0
    total_canon_drop = 0  # invalid after canonicalize
    rounds = 0
...
def main() -> None:
    ...
    model = SmilesLSTM(
        ...
        hidden_dim=m_args["hidden_dim"],
        num_layers=m_args["num_layers"],
        dropout=0.0,
        pad_id=tok.pad_id,
    ).to(device)
    ...
    out_path = pred_dir / f"v1_{tag}.txt"
    write_submission(sub, out_path)
    print(f"  wrote {out_path}")

    # ---- OFFICIAL aux-metric verification ----
    ...
    sum_path = pred_dir / f"v1_{tag}_summary.json"
